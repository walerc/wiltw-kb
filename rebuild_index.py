...
#wiltw-panel, #futures-panel, #ta-panel, #qa-panel, #trend-panel, #pattern-panel { display: none; }
#wiltw-panel.active, #futures-panel.active, #ta-panel.active, #qa-panel.active, #trend-panel.active, #pattern-panel.active { display: block; }
...
<div class="nav-tabs">
  ...
  <button class="nav-tab" onclick="switchTab('pattern')">📐 形态通道</button>
</div>
...
<div id="pattern-panel">
  <iframe id="patternFrame" src="pattern/index.html" style="width:100%;height:calc(100vh - 130px);border:none;"></iframe>
</div>
...
function switchTab(tab){
  ...
  document.querySelectorAll('#wiltw-panel,#futures-panel,#ta-panel,#qa-panel,#trend-panel,#pattern-panel').forEach(p=>p.classList.remove('active'));
  var idx=tab==='wiltw'?0:tab==='futures'?1:tab==='ta'?2:tab==='qa'?3:tab==='trend'?4:5;
  ...
  if(tab==='pattern'&&!document.getElementById('patternFrame').src)
    document.getElementById('patternFrame').src='pattern/index.html';
}
function init(){
